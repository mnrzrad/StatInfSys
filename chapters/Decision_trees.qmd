...
## Handling Missing Values in CART 
...
tree_clean <- rpart(Ozone ~ Solar.R + Wind + Temp + Month, data = aq_clean, method = "anova")

rpart.plot(tree_clean, type = 2, extra = 100, fallen.leaves = TRUE, box.palette = "BuGn")
```
...
# Identify missing Ozone entries
missing_idx <- is.na(airquality$Ozone)

# Predict them
predicted_ozone <- predict(tree_na, newdata = airquality[missing_idx, ])
...
# Combine results
airquality$Ozone[missing_idx] <- predicted_ozone
...
tree_nhanes <- rpart(chl ~ age + bmi + hyp,
                     data = nhanes,
                     method = "anova",
                     na.action = na.rpart)
rpart.plot(tree_nhanes, box.palette = "Purples", main = "CART with Surrogate Splits")
```
