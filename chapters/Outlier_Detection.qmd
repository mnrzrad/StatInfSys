...
As we discussed previously, a boxplot shows the distribution of a numerical variable using five key values -- the minimum, first quantile ($Q_1$), median, third quantile ($Q_3$), and maximum -- and highlights any suspected outliers based on the interquartile range ($\text{IQR} = Q_3 - Q_1$) rule. 
...
$$
where $x_i$ is individual observation, and $\text{MAD}=\text{median}(|x_i - \text{Median}|)$. 
