---
webr:
  show-startup-message: false # status of webR initialization
  show-header-message: false  # HTTP Header Check
  packages: # Pre-install dependencies
    - ggplot2
    - MASS
    - dplyr
filters: 
...
```{webr-r}
ggplot(data = mpg, mapping = aes(x = xty, y = hwy, colour = class)) +
  goem_point() + 
...
```{webr-r}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(aes(y = ..density..), binwidth = 2, fill = "lightblue", color = "white") +
...
In *ggplot2*, when you use `geom_histogram()`, the default $y$-axis is **count** (how many observations fall in each bin). But if you write `aes(y = ..density..)`, you are asking ggplot2 to scale the height of the bars so they represent probability density instead of raw counts. This allows you to compare this histogram with a density curve (like the one from `geom_density()`), which also shows density.
:::
...
geom_histogram(fill = "cornsilk", colour = "grey60", size = .2, bins = 30) +
geom_density() +
...
You can map density levels to **color** using `..level..`
...
library(corrplot)
library(faraway) # for loading gamb_data

gamb_data <- data.frame(teengamb)
corr_gamb <- cor(gamb_data)
corrplot(corr_gamb, tl.col = 'brown', tl.srt = 30, bg = 'white', type = 'full')
```
