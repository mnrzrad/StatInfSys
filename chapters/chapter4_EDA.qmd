...
To understand the **interquantile range (IQR),** first we define **quartiles**. Quartiles split the data into four equal parts: the first quartile (**Q1**) is the value below which 25% of observations fall, the second quartile (**Q2**) is the median (50%), and the third quartile (**Q3**) marks 75%. These three cut points summarize where the lower quarter ends, where the middle lies, and where the upper quarter begins, which is especially helpful when the distribution is not symmetric.
