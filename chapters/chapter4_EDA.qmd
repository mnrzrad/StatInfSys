...
\| Variable \| Is it categorical? \| Scale \| Justification \|
\|----------------\|--------------------\|---------\|--------------------------------------------------\|
\| \`manufacturer\` \| Yes \| Nominal \| It's a brand name, no natural order \|
\| \`model\` \| Yes \| Nominal \| Specific model name, not ordered \|
\| \`trans\` \| Yes \| Nominal \| Transmission type (e.g., auto, manual) \|
\| \`drv\` \| Yes \| Nominal \| Drive type (\`f\`, \`r\`, \`4\`) — no natural order \|
\| \`fl\` \| Yes \| Nominal \| Fuel type — categories like \`p\`, \`d\`, \`e\` \|
\| \`class\` \| Yes \| Nominal \| Vehicle class names (e.g., \`suv\`, \`compact\`) \|
\| \`year\` \| No \| — \| Numeric year — treated as a number, not category \|
\| \`displ\` \| No \| — \| Numeric — engine displacement \|
-->
````
:::
...
```{webr-r}
is.numeric(mpg$displ)    # Is it numeric?
is.integer(mpg$year)     # Is it stored as an integer?
is.double(mpg$cty)       # Is it a floating point number?
```
...
````{=html}
<!--
| Variable | Is numeric? | Type (Integer/Double) | Scale | Should it be categorical? | Justification |
|-----------|-----------|-----------|-----------|-----------|-----------------|
| `displ` | Yes | Double (`is.double()`) | Continuous | No | Engine size in litres, measured |
| `cty` | Yes | Double | Discrete\* | No | City MPG, looks discrete but is measured |
| `hwy` | Yes | Double | Discrete\* | No | Highway MPG, measured but in whole numbers |
| `cyl` | Yes | Double | Discrete | Maybe | Cylinders are countable — could be categorical |
| `year` | Yes | Integer | Discrete | Maybe | Only two years; may be better as factor |


``` r
is.numeric(mpg$cty)     # TRUE
is.integer(mpg$year)    # FALSE (even though it's a year!)
is.double(mpg$displ)    # TRUE
```

``` r
mpg$cyl <- as.integer(mpg$cyl)
mpg$year <- as.factor(mpg$year)   # if treating as categorical
```
-->
````
:::
...
# Count how many missing values (NA) exist in each column
colSums(is.na(mpg))

# Check if there are any missing values at all in the dataset
anyNA(mpg) 
```

::: callout-tip
### R Tip: `any()`, `all()`, and `anyNA()`
...
![](/figs/EDA/normal.jpg){fig-align="center"}
...
![\[source:https://towardsdatascience.com/skewness-kurtosis-simplified-1338e094fc85/\]](/figs/EDA/skew.png){fig-align="center"}
...
![\[Source: https://towardsdatascience.com/skewness-kurtosis-simplified-1338e094fc85/\]](/figs/EDA/kurtosis.png){fig-align="center"}
...
![\[Source: https://medium.com/\@the_social_byte/why-should-we-care-about-skewness-and-kurtosis-in-data-analysis-bf6bff4b4ab6\]](/figs/EDA/both.png)
...
## 2x2 plots
par(mfrow = c(2, 2))  # (not mfrom)

plot(dataset1$x, dataset1$y, xlim = c(4, 19), ylim = c(3, 13),
     pch = 19, main = "Dataset I", xlab = "x", ylab = "y")
# abline(lm(y ~ x, data = dataset1), lty = 2)  # optional

plot(dataset2$x, dataset2$y, xlim = c(4, 19), ylim = c(3, 13),
     pch = 19, main = "Dataset II", xlab = "x", ylab = "y")
# abline(lm(y ~ x, data = dataset2), lty = 2)

plot(dataset3$x, dataset3$y, xlim = c(4, 19), ylim = c(3, 13),
     pch = 19, main = "Dataset III", xlab = "x", ylab = "y")
# abline(lm(y ~ x, data = dataset3), lty = 2)

plot(dataset4$x, dataset4$y, xlim = c(4, 19), ylim = c(3, 13),
     pch = 19, main = "Dataset IV", xlab = "x", ylab = "y")
# abline(lm(y ~ x, data = dataset4), lty = 2)
```
...
|:---------------|:----------------------------------:|:----------------------------------------:|:--------------------------------:|
| **Nominal**   | Cramér’s V<br>φ coefficient (binary)<br>Tetrachoric corr. (binary) | No standard measure                | Eta (η)                        |
| **Ordinal**       | No standard measure         | Spearman’s ρ<br>Kendall’s τ           | Spearman’s ρ<br>Kendall’s τ   |
| **Numeric**       | Eta (η)                         | Spearman’s ρ<br>Kendall’s τ           | Pearson’s r<br>Spearman’s ρ<br>Kendall’s τ                   |
...
2.  Compute the **mean**, **median**, and **standard deviation** of `Ozone`, `Wind`, and `Temp`.

```{webr-r}

```

3.  For `Month`, create a frequency table. Which month has the most observations?
...
2.  Compute the Spearman correlation between`Ozone` and `Solar.R`.

```{webr-r}

```

3.  Compute the correlation between `Ozone` and `Month`

```{webr-r}

```

4.  Which factor (Temp, Wind, Solar.R, or Month) seems most strongly associated with Ozone levels?
